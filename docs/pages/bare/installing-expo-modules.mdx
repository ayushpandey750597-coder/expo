...
- [`expo-font`](/versions/latest/sdk/font) - Load fonts at runtime. This module is optional and can be safely removed, however; it is recommended if you use `expo-dev-client` foake`](/versions/l}
}
```cd MyLearningApp
npm install
npx expo install expo-linear-gradient @react-native-async-storage/async-storage @expo/vector-icons react-native-screens react-native-safe-area-context
